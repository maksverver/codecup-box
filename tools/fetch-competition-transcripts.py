...
GAME_RE = re.compile(r'<script type="application/javascript">var g = new Box\((\[.*?]), (\[.*?]), (\[.*?])\);</script>')
...
def FetchGame(cachedir, game_id):
  ...
  if m is None:
    ...
    return None
  users, moves, colors = map(json.loads, m.groups())
  assert len(users) == len(colors) == 2
  assert len(moves) > 0
  assert all(isinstance(elem, str) for elem in users)
  assert all(isinstance(elem, str) for elem in moves)
  assert all(isinstance(elem, int) for elem in colors)
  return (users, moves, colors)


def WriteAnnotatedCsv(filename, games):
  with open(filename, 'wt', newline='') as f:
    w = csv.writer(f, lineterminator='\n')
    w.writerow(('Game', 'Round', 'IsSwiss', 'User1', 'Score1', 'Status1', 'User2', 'Score2', 'Status2', 'Color1', 'Color2', 'Moves'))
    for game in games:
      w.writerow((game.game_id, game.round, game.is_swiss, game.user1, game.score1, game.status1, game.user2, game.score2, game.status2, game.color1, game.color2, game.moves))
  ...
  if len(sys.argv) != 4:
    ...
    for i, game in enumerate(games):
      ...
      if details is not None:
        users, moves, colors = details
        assert users == [game.user1, game.user2]
        game.moves = ' '.join(moves)
        game.color1, game.color2 = colors
